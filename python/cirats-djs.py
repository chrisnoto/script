...
first_page=res2.content
next_page=''
soup=BeautifulSoup(first_page)
if soup.find_all(href=re.compile('PAGING')):
	page_num=soup.find_all(href=re.compile('PAGING'))[-2].string
	for num in range(2,int(page_num)):
		values3={'POPUP_CLOSED':'',
        		'EFLUX_MODE':'Patch-advisories-list',
       			'EFLUX_STATE':'0',
        		'eflux-crumb-count':'2',
        		'eflux-show-navigation':'true',
        		'AUTO_REFRESH':'format-only',
        		'windowstatus':'0',
        		'Command':'',
        		'hidden1':'siva',
        		'sortBy':'recordLocatorFilter.R.RECORD_ID',
        		'recordLocatorFilter_PAGEID':str(num),
        		'Mode':'Patch-advisories-list',
        		'FROM_ACTIVITY':'PAGING',
        		'PAGE_REFRESH':'false'
		}

		res3=s.post(url1,data=values3,headers=headers)
#res3=s.get(url2,headers=headers)

		next_page=next_page+ res3.content
html=first_page+next_page
soup=BeautifulSoup(html)
for tds in soup.select('.odd td'):
	if not len(tds.string) == 1:
